...
const SelectLogoColor = () => {
     const { colorsInLogo, setColorsInLogo, selectedVariants } = useContext(SettingsContext)
     console.log("🚀 ~ SelectLogoColor ~ colorsInLogo:", colorsInLogo)
     const handleSelectedColor = (v:any, item:any) => {
          const newVariant = { variantName: v, colorInLogo: item};
          setColorsInLogo([...colorsInLogo, newVariant])
     }
     ...
     return (
          <>
               ...
               <section className='bg-gray-50 md:p-6 p-5 border-[1.5px] rounded-lg border-secondary mt-4'>
                    {
                         selectedVariants?.map((v: any, id: number) => (
                              <div key={id}>
