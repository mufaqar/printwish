...
const SelectLogoColor = ({InvokeCalculatePrice}:any) => {
               ...
               <div className='flex items-center justify-center md:justify-start md:items-start flex-wrap mt-4 gap-2 p-0 '>
                    {
                         [1, 2, 3, 4, 5].map((item: any, idx: number) => (
                              <div className='relative' key={idx} >
                                   <button onClick={() => {setColorsInLogo(item); sessionStorage.setItem('colorInLogo', item); InvokeCalculatePrice(item)}} className={` p-2 cursor-pointer px-4 pl-2 text-lg bg-white rounded-full border-[2px] ${colorsInLogo === item ? ' border-secondary' : 'border-gray-100 hover:border-main'}`}>
