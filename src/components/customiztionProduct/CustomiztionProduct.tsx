...
const CustomiztionProduct = () => {
     const  { setSelectedCustomizedLayout} = useContext(SettingsContext)
     const [CustomizationType, setCustomizationType] = useState('')
     const [nameAndNumber, setNameAndNumber] = useState<any>(false)
          ...
          <>
               <section className='mt-4 bg-background p-8  rounded-lg'>
                    ...
                    <div className='mt-5 md:flex item-center gap-6'>
                         ...
                         <button onClick={() => { setCustomizationType('Embroidery'); setNameAndNumber(false); }} className={`bg-white flex items-center gap-3 p-2 hover:scale-105 active:scale-100 mt-3 md:mt-0 w-full md:w-60 text-xl border-[2px] rounded-full ${CustomizationType === 'Embroidery' ? 'border-secondary' : ' border-transparent'}`}>
                              <figure className='bg-accent p-2 rounded-full'>
               ...
               {CustomizationType.length > 1 &&
                    <section className='mt-4 bg-background p-8 rounded-lg'>
                              ...
                              <h5 className='text-xl font-semibold text-accent font-roboto'>Step 4 - Design Position:</h5>
                              {
                                   CustomizationType === 'Print' && <div className='flex items-center gap-2'>
                                        ...
                                        <label className="relative inline-flex items-center cursor-pointer">
                                             <input type="checkbox" value={nameAndNumber} onChange={() => {setNameAndNumber(!nameAndNumber);setSelectedCustomizedLayout('')} } className="sr-only peer" />
                         ...
                         {
                              nameAndNumber ? <NameAndNumber /> : <DesignPosition />
                         }
                    </section>
               }
          </>
